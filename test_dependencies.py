...
try:
    ...
    try:
        hf_hub_download(repo_id=training_config.model_name, filename="config.json", use_auth_token=True)
        print(f"✅ Access to model '{training_config.model_name}' confirmed.")
    except Exception as model_error:
        raise PermissionError(
            f"Token is valid, but does not have access to model '{training_config.model_name}': {model_error}. You must request access on HuggingFace.")
